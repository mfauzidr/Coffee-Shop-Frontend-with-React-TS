  ...
  return (
    <form className="flex flex-col" action="" onSubmit={handleSubmit}>
      ...
      <div className="grid grid-cols-4 gap-2 mb-2">
        {images.length > 0 ? (
          images.map((src, index) => (
            <div
              key={`${src}-${index}`}
              className="relative w-full h-20 rounded-xl border bg-white overflow-hidden"
            >
              <img
      ...
      <input
        ...
        name="name"
        type="text"
        placeholder={detailProduct.productName || `Enter Product Name`}
        onChange={onChange}
      />
      ...
      <input
        ...
        name="price"
        type="number"
        placeholder={String(detailProduct.price) || `Enter Product Price`}
        min={0}
        onChange={onChange}
      />
      ...
      <textarea
        ...
        id="description"
        name="description"
        placeholder={detailProduct.description || `Enter Product `}
        onChange={onChange}
      />
          ...
          {sizes?.map((option, index) => (
            <li key={option.value} className="flex w-20 h-10">
              <input
                ...
                defaultChecked={detailProduct.sizeId === option.id}
              />
              <label
          ...
          {categories?.map((option, index) => (
            <li key={option.value} className="flex w-20 h-10">
              <input
                ...
                defaultChecked={detailProduct.categoryId === option.id}
              />
              <label
