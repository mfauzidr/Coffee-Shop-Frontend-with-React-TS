    ...
    <div className="flex-1 flex-col relative">
      ...
      <div className="relative flex-1 bg-white shadow-md mx-3 lg:mx-6 p-3 -mt-8">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex-1 h-14 lg:h-auto text-xs lg:text-sm overflow-scroll-hidden">{description}</div>
          {ratingProduct && (
          ...
          <div className="flex-col lg:flex text-sm lg:text-2xl text-amber-500">
            {isFlashSale ? (
              <div>
                <span className="block lg:flex text-[8px] lg:text-xs font-bold text-red-500 line-through">IDR {price?.toLocaleString('id')}</span>
                IDR {discount.toLocaleString('id')}
              </div>
            ) : (
              <span className='text-xs lg:text-md'>{`Rp. ${price?.toLocaleString('id')} ,-`}</span>
            )}
          </div>
          <div className="flex flex-1 gap-2">
            <Button buttonName='Buy' type='button' size='w-full' link={`/detail-product/${uuid}`} />
            <CartButton size='14px lg:24px' padding='px-4 py-px' border='border border-amber-500 rounded' />
          </div>
        </div>
      </div>
    </div>
