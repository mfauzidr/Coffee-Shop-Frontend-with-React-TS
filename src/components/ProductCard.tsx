...
const ProductCard: React.FC<ProductCardProps> = ({ uuid, image, productName, description, price, isFlashSale, ratingProduct }) => {
  ...
  return (
    <div className="flex-1 flex-col relative">
      {isFlashSale && (
        <div className="absolute top-2 left-2 bg-red-600 text-white text-xs md:text-lg px-2 py-1 rounded-3xl">
          Flash Sale!
        </div>
      )}
      <div className="flex flex-1 h-32 md:h-64 min-w-40 md:w-full justify-center overflow-hidden">
        <img className='flex-1 object-cover object-center' src={image ? `${image}` : defaultImage} alt={productName} />
      </div>
      <div className="relative flex-1 bg-white shadow-md mx-3 md:mx-6 p-3 -mt-8">
        <div className="flex flex-col gap-2">
          <h2 className="text-xs md:text-2xl font-bold">
            <Link to={`/detail-product/${uuid}`}>{productName}</Link>
          </h2>
          <div className="flex-1 h-14 md:h-auto text-xs md:text-sm overflow-scroll-hidden">{description}</div>
          {ratingProduct && (
            <RatingStar gap={'gap-2'} rating={ratingProduct} />
          )}
          <div className="flex-col md:flex text-sm md:text-2xl text-amber-500">
            {isFlashSale ? (
              <div>
              ...
              <span className='text-xs md:text-md'>{`Rp. ${price?.toLocaleString('id')} ,-`}</span>
            )}
          </div>
